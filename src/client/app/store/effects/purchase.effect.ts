    ...
    public startTransaction = this.actions.pipe(
        ...
        mergeMap(async (payload) => {
            try {
                ...
                await this.cinerinoService.getServices();
                const passport =
                    params.object.passport === undefined
                        ? await this.cinerinoService.getPassport({
                              scope: `Transaction:PlaceOrder:${params.seller.id}`,
                          })
        ...
        mergeMap(async (payload) => {
            try {
                ...
                if (
                    ...
                    throw new Error(
                        'seller.id or auth.options.clientId undefined'
                    );
                }
                await this.cinerinoService.getServices();
                const screeningEventTicketOffers =
                    await this.cinerinoService.event.searchTicketOffers({
                        ...
                        store: {
                            id: this.cinerinoService.auth.options.clientId,
                        },
                    });
            ...
            try {
                ...
                if (payload.authorizeCreditCardPayment !== undefined) {
                    await this.cinerinoService.payment.voidTransaction(
                        payload.authorizeCreditCardPayment
                    );
                }
                const transaction = payload.transaction;
                ...
                return purchaseAction.authorizeCreditCardSuccess({
    ...
    public createCreditCardToken = this.actions.pipe(
        ...
        mergeMap(async (payload) => {
            ...
            try {
                ...
                return purchaseAction.createCreditCardTokenSuccess({
                    creditCardToken,
                });
            } catch (error) {
                return purchaseAction.createCreditCardTokenFail({
                    error: error,
                });
            }
        })
                        ...
                        sender: {
                            name:
                                this.translate.instant(
                                    ...
                                    ? authorizeSeatReservations[0].event
                ...
                if (environment.ANALYTICS_ID !== '') {
                    ...
                    try {
                        if (
                            ...
                            throw new Error(
                                'reservationType is not EventReservation'
                            );
                        }
                        const itemOffered = <
                        ...
                        const sendData = {
                            ...
                            eventLabel:
                                itemOffered.typeOf ===
                                factory.chevre.reservationType.EventReservation
                                    ? `conversion-${itemOffered.reservationFor.superEvent.location.branchCode}`
                                    : 'conversion',
                        };
                        ga('send', sendData);
                    } catch (error) {
                ...
                if (linyId !== undefined) {
                    ...
                    try {
                        const view = await this.utilService.getText(
                            ...
                            }/ejs/liny/complete/${payload.language}.ejs`
                        );
    ...
    public convertExternalToPurchase = this.actions.pipe(
        ...
        mergeMap(async (payload) => {
            ...
            try {
                ...
                return purchaseAction.convertExternalToPurchaseSuccess({
                    screeningEvent,
                    seller,
                    theater,
                });
            } catch (error) {
                return purchaseAction.convertExternalToPurchaseFail({
                    error: error,
                });
            }
        })
