...
export class PurchaseSeatTicketModalComponent implements OnInit {
    ...
    @Input()
    public checkMovieTicketActions: factory.action.check.paymentMethod.movieTicket.IAction[];
    ...
    public ngOnInit() {
        ...
        this.screeningEventTicketOffers.forEach((ticketOffer) => {
            ...
            >ticketOffer.priceSpecification.priceComponent
                .filter(
                    (c) =>
                        c.typeOf ===
            ...
            this.checkMovieTicketActions.forEach((checkMovieTicketAction) => {
                ...
                const availabilityMovieTickets =
                    checkMovieTicketAction.result.movieTickets.filter(
                        (movieTicket) => {
                            return (
                                (<any>movieTicket).validThrough === undefined
                            );
                        }
                    );
                availabilityMovieTickets.forEach((movieTicket) => {
                    if (
                        movieTicket.serviceType ===
                        movieTicketTypeChargeSpecification.appliesToMovieTicket
                            ?.serviceType
                    ) {
            ...
            targetMovieTickets.forEach((movieTicket) => {
                const reservationsIndex = reservations.findIndex(
                    (reservation) => {
                        return (
                            reservation.ticket !== undefined &&
                            reservation.ticket.movieTicket !== undefined &&
